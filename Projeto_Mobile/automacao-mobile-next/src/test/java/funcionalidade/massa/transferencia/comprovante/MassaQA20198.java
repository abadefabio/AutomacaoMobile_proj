...
public class MassaQA20198 extends MassaQABase implements CasoDeTesteMassa {

    @Override
    public JSONObject criar() throws Exception {
        JSONObject json, arquivoJsonMassa = obterJsonTesteAtual();

        if (arquivoJsonMassa.get("criarMassaAgora").equals(false)) {
            return arquivoJsonMassa;
        }

        json = new PortalAdesaoNext().criacaoAdesaoAtivadoComTutorial(arquivoJsonMassa, StatusAdesaoOnboarding.APROVADA_FINALIZADA);
